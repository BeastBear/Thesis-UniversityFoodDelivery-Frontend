...
function CancelOrderPending() {
  ...
  const handleShopBusyCancel = async () => {
    ...
    try {
      ...
      // Calculate reopen time: 1 hour from now
      const now = new Date();
      const reopenTimeDate = new Date(now.getTime() + 60 * 60 * 1000); // Add 1 hour
      const reopenTime = `${String(reopenTimeDate.getHours()).padStart(2, "0")}:${String(reopenTimeDate.getMinutes()).padStart(2, "0")}`;

      // Close shop temporarily for 1 hour
      await axios.post(
            ...
            <h1 className="text-xl font-bold text-gray-900">
              {selectedReason === "out_of_stock"
                ...
                : selectedReason === "shop_busy"
                  ? "Restaurant is busy (closed for 1 hr.)"
